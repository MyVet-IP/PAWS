export function initDashboard() {
  ...
  if (nameEl) nameEl.textContent = `Bienvenido, ${user.nombre.split(' ')[0]}!`;
    ...
    .then(data => {
      ...
      const cards = pets.map(p => `
        <div class="bg-white rounded-2xl shadow-md w-64 p-5 hover:-translate-y-1 transition">
          ...
          <p class="text-gray-500 text-sm mb-4">${p.raza || p.especie} • ${p.edad} ${p.edad === 1 ? 'año' : 'años'}</p>
          <button class="bg-softpink w-full py-2 rounded-full font-medium">Ver perfil</button>
        </div>
      `).join('');
  ...
  <section class="flex h-screen bg-gray-50">
    ...
    <main class="flex-1 p-10 overflow-y-auto">
      ...
      <div class="flex justify-between items-center mb-8">
        <div>
          <h1 id="dash-username" class="text-3xl font-bold text-gray-800">Bienvenido!</h1>
      ...
      <div class="grid grid-cols-3 gap-6 mb-10">
        ...
        <h2 class="text-xl font-semibold text-gray-800 mb-6">Tus mascotas</h2>
  ...
  <div id="modal-add-pet" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-3xl p-8 w-full max-w-md shadow-xl">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-bold text-gray-800">Agregar mascota</h2>
      ...
      <form id="add-pet-form" class="space-y-4">
        <div>
          ...
          <input id="pet-nombre" type="text" required placeholder="Ej. Bruno"
        ...
        <div>
          <label class="text-sm text-gray-600">Raza</label>
          <input id="pet-raza" type="text" placeholder="Ej. Labrador"
            class="mt-1 w-full px-4 py-2 rounded-full border focus:outline-none focus:ring-2 focus:ring-pink">
        </div>
        <div>
          <label class="text-sm text-gray-600">Edad (años)</label>
